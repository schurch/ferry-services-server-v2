...
import           Amazonka                            (AWSRequest, AWSResponse,
                                                      LogLevel (..), Logger,
                                                      Region (Sydney), newEnv,
                                                      runResourceT, send)
import           Amazonka.Auth                       (AccessKey (..),
                                                      SecretKey (..), envLogger,
                                                      envRegion, fromKeys)
import           Amazonka.SNS                        (_EndpointDisabledException,
                                                      _NotFoundException)
import           Amazonka.SNS.CreatePlatformEndpoint (createPlatformEndpointResponse_endpointArn,
                                                      newCreatePlatformEndpoint)
import           Amazonka.SNS.DeleteEndpoint         (newDeleteEndpoint)
import           Amazonka.SNS.GetEndpointAttributes  (getEndpointAttributesResponse_attributes,
                                                      newGetEndpointAttributes)
import           Amazonka.SNS.Publish                (newPublish,
                                                      publish_messageStructure,
                                                      publish_targetArn)
import           Amazonka.SNS.SetEndpointAttributes  (newSetEndpointAttributes,
...
updateDeviceTokenForEndpoint logger endpointARN deviceToken = do
  let request =
        newSetEndpointAttributes (pack endpointARN) & setEndpointAttributes_attributes .~ fromList
